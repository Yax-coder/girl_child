...
class LessonPlayerScreen extends StatelessWidget {
  ...
  const LessonPlayerScreen({super.key, required this.initialSession});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => SessionBloc(context.read(), context.read())
        ..add(SessionStarted(initialSession)),
...
class _LessonPlayerViewState extends State<_LessonPlayerView> with SingleTickerProviderStateMixin {
    ...
    return Scaffold(
      ...
      body: BlocBuilder<SessionBloc, SessionState>(
        builder: (context, state) {
          if (state.currentStep == null || state.isLoading) {
            ...
            children: [
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                      // Jidda Avatar - Always Present for Encouragement
                      _buildJiddaAvatar(),
                      const SizedBox(height: 40),
                      
                      // Progress Indicator (Minimalist)
                      _buildMiniProgress(step),
                      ...
                      if (step.currentStep == "QUIZ") ...[
                        const SizedBox(height: 32),
                        _buildQuizOptions(context, step.stepContent),
                      ],
              ...
              _buildBottomAction(context, step),
            ],
          );
        },
      ),
  ...
  Widget _buildJiddaAvatar() {
    return Stack(
      ...
      children: [
        ...
        // Animated Voice Wave "Listening/Talking" Indicator
        Positioned(
          bottom: 0,
          child: Container(
              ...
              builder: (context, child) {
                return Row(
                  ...
                  children: List.generate(3, (index) {
                    return Container(
                      width: 4,
                      height: 8 + (_waveController.value * 12 * (index == 1 ? 1 : 0.5)),
  ...
  Widget _buildMiniProgress(SessionStepResponse step) {
    final steps = ["INTRO", "CONTENT", "PRACTICE", "QUIZ", "REFLECTION", "COMPLETE"];
    final currentIndex = steps.indexOf(step.currentStep);
  ...
  Widget _buildQuizOptions(BuildContext context, StepContent content) {
    ...
    return Column(
      children: List.generate(content.options!.length, (index) {
        return Padding(
          ...
          child: InkWell(
            onTap: () => context.read<SessionBloc>().add(StepSubmitted(data: {'answerIndex': index})),
  ...
  Widget _buildBottomAction(BuildContext context, SessionStepResponse step) {
        ...
        child: ElevatedButton(
          onPressed: () {
            if (isFinal) {
              ...
              context.read<SessionBloc>().add(StepSubmitted(data: {'reflectionText': _reflectionController.text}));
            } else {
              context.read<SessionBloc>().add(StepSubmitted());
            }
          },
          ...
          child: Row(
            ...
            children: [
              Text(
                isFinal ? "Gama Wannan Tafiya" : "Ci gaba",
  ...
  String _getTitleForStep(SessionStepResponse step) {
    ...
    switch (step.currentStep) {
      case "INTRO": return content.lessonTitle ?? "Sannu da zuwa!";
      case "CONTENT": return "Darasi (Lesson)";
      case "PRACTICE": return "Gwaji (Practice)";
      case "QUIZ": return "Tambaya (Quiz)";
